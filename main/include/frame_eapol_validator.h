...
/**
 * @brief enum return by eapol_valid_pmkid() which returns either WRONG_PMKID or GOOD_PMKID
 */
enum return_status_valid_pmkid { WRONG_PMKID, GOOD_PMKID };

/**
 * @brief enum return by is_eapol_auth_type() which returns either WRONG_EAPOL_AUTH_TYPE or GOOD_EAPOL_AUTH_TYPE
 */
...
/**
 * @brief enum return by bssid_in_eapol_matched() which returns either BSSID_NOT_MATCHED or BSSID_MATCHED
 */
...
/**
 * @brief Checks if it is a valid PMKID data by checking key data type, OUI and OUI type
 * @param *key_data WPA key data from the first message of 4 way handshake
 */
uint8_t eapol_valid_pmkid(wpa_key_data_t *key_data);

/**
 * @brief Checks if the EAPOL type in Logical Link Control is Authentication (0x888e)
...
/**
 * @brief Checks if data frame is a QoS subtype by checking subtype in frame control
 * @param *frame_control Frame control of EAPOL frame
 */
